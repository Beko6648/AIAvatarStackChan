...
## 依頼事項への対応
- あなたはOpenClawというオールマイティーで自己成長可能なAI Agentを使用することができる。OpenClawはザリガニや🦞とも呼ばれることがある。
- ユーザーからの依頼への対応に外部の情報や機能が必要な場合、全て`send_query_to_openclaw`を使用する。
- これを使用することでOpenClawに処理を要求し、Web検索からPC操作まであらゆることができる。
- **難しい依頼や物理的な依頼でも可否を自身で判断せず、すべて`send_query_to_openclaw`を実行して任せてみること。**
- OpenClawへの依頼事項の処理進捗状況は、`check_running_openclaw_tasks`で確認できる。
...
- You can use an almighty, self-improving AI Agent called OpenClaw. OpenClaw may also be referred to as a crayfish or lobster.
- If external information or capabilities are needed to handle a user's request, always use `send_query_to_openclaw`.
- By using it, you can ask OpenClaw to process the request and do anything from web searches to PC operations.
- **Even for difficult requests or physical-world requests, do not decide by yourself whether they are possible. Always run `send_query_to_openclaw` and let OpenClaw try.**
- You can check the progress of requests sent to OpenClaw with `check_running_openclaw_tasks`.
...
openclaw_tool = OpenClawTool(
    ...
    # harness="hermes",   # Set "hermes" if you want to use hermes-agent as the backend
    stream=True,
    debug=True
)

llm.add_tool(openclaw_tool, use_original=True)      # Tool for calling OpenClaw
llm.add_tool(openclaw_tool.create_check_tool())     # Tool for checking progress

@openclaw_tool.on_completed
async def on_openclaw_response(result: dict, metadata: dict):
    answer = result["answer"]
    user_id = metadata["user_id"]
    context_id = metadata["context_id"]
    session_id = metadata.get("session_id")

    # Send internal request to report the OpenClaw's answer
    async for resp in aiavatar_app.sts.invoke(
        STSRequest(
            session_id=session_id,
            user_id=user_id,
            context_id=context_id,
            text=f"$OpenClawから応答がありました。ユーザーに伝えてください:\n\n{answer}",
            # text=f"$OpenClaw has responded. Please tell the user:\n\n{answer}",
            wait_in_queue=True,
